 ...
 *
 * All state lives in Sync (see sync.js). Nothing here keeps its own copy, so
 * when Sync starts talking to Firebase this file keeps working unchanged.
 */
(function () {
  'use strict';
  ...
  function render(state) {
    ...
    startBtn.disabled = state.status === 'running' || (left <= 0 && state.status !== 'alarming');
    startBtn.textContent = state.status === 'paused' ? 'Resume' : 'Start';
    pauseBtn.disabled = state.status !== 'running';
    stopBtn.disabled = state.status === 'idle' && left === state.durationMs;
